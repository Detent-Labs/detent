...
| `src/http/` | Thin REST/JSON wrapper over `Bun.serve` around the Runtime API Layer. One route file per surface: participant, admin, studio, reporting and account. The UI-string routes sit in `admin-routes.ts`, and `server.ts` answers `/livez` and `/readyz` itself. Beside them, `metrics.ts` holds the scrape and `static.ts` serves the frontend from `WEB_ROOT`. Typed-error-to-HTTP-status mapping, configurable CORS. |
...
| `src/tenancy/` | An optional database-per-tenant mode. `TENANT_CONTROL_PLANE_URL` selects it; unset, the server runs single-tenant against `DATABASE_URL`. A control plane lists the tenants (`store.ts`), `connections.ts` maps a key to its own database, and `provision.ts` creates one. Only the CLI adds a tenant; no HTTP route does. |
...
| `examples/expense-approval.json` | Complete Capture → Review → Book example. Runs end-to-end in the devcontainer against the webhook sink backgrounded inside the `app` container. |
