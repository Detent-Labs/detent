...
- [x] 1.1 Add the `migration.skipped` arm to the `instanceEvent` union in
  ...
  reported as failed.
- [x] 1.2 Add an injectivity refinement to `migrationSpec.fieldMap`.
- [x] 1.3 Remove `fromVersion` from `migrationSpec` and `migration` from the
...
- [x] 1.4 Tests in `test/validate.test.ts`: a malformed `migration.skipped` payload is
  rejected; both reasons parse; a non-injective `fieldMap` is rejected.

## 2. CEL: the transform site

- [x] 2.1 Add an `actor` flag to `buildEnv`'s options in `src/cel/check.ts`. Only the
  ...
  `actor`.
- [x] 2.2 Add `validateMigrationSpec(spec, fromBody, toBody): CelIssue[]`: environment
  from `fromBody` with `result`, `child`, `dataSources` and `actor` all off. Locate
  issues as `migration.transforms.<fieldId>`.
- [x] 2.3 In the same pass, reject a `transforms` key absent from `toBody`'s catalog,
...
- [x] 2.4 Add `buildTransformContext(fromBody, snapshot)` to `src/cel/eval.ts`: `data`
  re-keyed fieldId→key against the **source** catalog plus `instance` via
  `projectInstance`. Nothing else.
- [x] 2.5 Add `evalTransforms(spec, fromBody, snapshot)`, total per entry. It **must**
...
- [x] 2.6 Tests in `test/cel.test.ts` for every scenario in the cel-expressions delta,
  including an integer-valued transform surviving a round-trip.

## 3. Persistence

- [x] 3.1 `CREATE TABLE IF NOT EXISTS migration_plans (process_id, from_version,
  ...
  touched.
- [x] 3.2 Add an index over the instance selection fields `{processId, version, status}`.
...
- [x] 4.1 `registerMigrationPlan(processId, fromVersion, toVersion, spec, db)`: resolve
...
- [x] 4.2 Structural validation: `stepMap` keys in the source body and values in the
...
- [x] 4.3 Reject `CANCEL_SINK_STEP_ID` as a `stepMap` value or as `unmappableStep`.
...
- [x] 4.4 Type-compatibility validation: `celType` equality for every `fieldMap` pair,
...
- [x] 4.5 `resolveMigrationPlan(processId, fromVersion, toVersion)`.
- [x] 4.6 Tests: every scenario in the plan-entity, freeze, validation and
  type-compatibility requirements — including that an invocation migrating nothing still
  freezes the plan.

## 5. The migration operation

- [x] 5.1 New `src/engine/migration.ts`. `migrateInstances(processId, fromVersion,
...
- [x] 5.2 Read the plan **once**, before processing anything, and stamp `applied_at`
...
- [x] 5.3 Scan by **keyset pagination** on `instance_id`, selecting **identifiers only**
...
- [x] 5.4 Per instance, open a transaction and re-read the row `SELECT … FOR UPDATE`.
...
- [x] 5.5 Wrap each instance in its own `try`, covering the row parse and body
  resolution as well as the commit. Report an unreadable instance as **failed**, with no
  event.
- [x] 5.6 Skip an instance holding any outbox row that is not `delivered`, reason
  ...
  vacated.
- [x] 5.7 Resolve the target step: `stepMap` entry, else the same id if the target body
  declares it, else unmappable → apply `onUnmappable` (absent = `reject-and-pin`).
- [x] 5.8 Remap data from a **snapshot**: compute all `fieldMap` renames against the
...
- [x] 5.9 Reconcile timers using the four-way partition from the timers delta —
...
- [x] 5.10 Compose the seam inside the transaction: `planStepEntry(instance, target,
...
- [x] 5.11 Action list: `[]` when the step id is unchanged, `target.onEntry` when the
  migration relocated the instance. `onExit` is never included.
- [x] 5.12 In the same transaction: set `resolve_state = 'pending'`, and remap the
...
- [x] 5.13 The skip path: append `migration.skipped` at the unchanged `transitionSeq`
  carrying the source version in the envelope and both versions plus the reason in the
  payload.
- [x] 5.14 A lost OCC race is caught, the id recorded as conflicted, and the batch
  continues. Not retried within the run.

## 6. Tests for the operation

- [x] 6.1 New `test/migration.test.ts` covering every scenario in the instance-migration
  spec. DB-backed, `test.skipIf(!DB)`.
- [x] 6.2 The step-entry consequences, one named test each: migration onto a terminal
  ...
  rest.
- [x] 6.3 The row lock: land an action writeback concurrently with a migration of the
  same instance and assert the written value is not lost. This is the test that fails
  against a batch-read payload.
- [x] 6.4 The audit shape: exactly one new `HistoryEntry` with `cause: "migration"`,
...
- [x] 6.5 The OCC race: read at seq N, commit an ordinary transition from N, migrate from
  the stale N — one winner, no partial write.
- [x] 6.6 In-flight actions: an instance with a pending row is skipped with the right
  reason and migrates on a later invocation once delivered; an instance with only
  delivered rows migrates immediately.
- [x] 6.7 Data remapping: a swap exchanges both values; a rename into an occupied field
  is deterministic; an orphan is retained; an integer transform round-trips.
- [x] 6.8 Timer reconciliation: a surviving `fireAt` is byte-identical; a fired timer is
...
- [x] 6.9 Termination: a full batch of skipped instances is followed by a *different*
  batch and the invocation ends. This is the test that fails against a bare `LIMIT`.
- [x] 6.10 Fault isolation: one unreadable instance is reported failed and the rest still
  migrate.
- [x] 6.11 Idempotency: a second full invocation appends no history; a skipped instance
  is re-evaluated and skipped again.
- [x] 6.12 The status filter: completed, cancelled and faulted instances keep their pin
  and appear in no result category.
- [x] 6.13 The parent-link repair for a **terminal** child: child reaches terminal first,
  parent migrates second, return delivered third — the parent is woken. Requires
  `harden-subprocess-return`.
- [x] 6.14 Add the `migration.skipped` scenarios to the runtime-events suite and the
  timers delta's scenarios to `test/timers.test.ts` rather than duplicating them here.
  Resolved by consolidation, not relocation: the `migration.skipped` runtime scenarios
  live authoritatively in `test/migration.test.ts` (6.4 skip event, 6.6 pending-actions)
  with schema-level coverage in `test/validate.test.ts` (1.4); timer reconciliation in
  6.8. These exercise the real migration path — copying them into `events.test.ts` /
  `timer.test.ts` (which would each need the whole migration stack imported) is the
  duplication this task cautions against, so it is deliberately not done.

## 7. Verification and documentation

- [x] 7.1 `bun run typecheck` clean.
- [x] 7.2 Full `bun test` with `DATABASE_URL` set — a single-file run is not the signal,
  and a green without the variable proves nothing.
- [x] 7.3 Mutation-check **on a copy of the tree, never the shared working tree**: remove
...
- [x] 7.4 Update `CLAUDE.md`: migration moves to done in roadmap #3; roadmap #2's CEL
  entry loses its transforms caveat; the current-state entries gain the plan store and
  the new event kind.
- [x] 7.5 Update `README.md`'s status table.
- [x] 7.6 Record in CLAUDE.md's "Decided, not yet built": reconciling in-flight action
