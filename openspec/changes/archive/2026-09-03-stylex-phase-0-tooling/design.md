...
| 3 studio, non-canvas | About 305 rules, about 750 sites, 6 test files. Pseudo-element `::backdrop` sees first use. This row's own original claim that `:popover-open` also does is wrong. Phase 0 already used it for the shell account menu. Corrected in `stylex-phase-3-studio`'s own task 10.2. Deltas against `studio-app`, `studio-form-editor` and the panel specs. | Probes pass on form editor, panels, dock and dialogs. Keyboard walks unchanged. |
