...
calls seven functions in sequence: `checkReservedActionPrefix`,
`checkUnknownKeys`, `checkPatterns`, `checkIdResolution`,
`checkColumnMapping`, `checkFieldKeyFormat`, `checkLengthBounds`.

`walkFieldsIndexed(body.fields, ...)` runs at six call sites in
`compile.ts`, not four. Four run one check each: `checkPatterns`,
`checkColumnMapping`, `checkFieldKeyFormat`, and `checkLengthBounds`'s
own field-key-length loop. Two more call it too. `checkUnknownKeys`
walks fields through its `walkFieldDefKeys` callback.

`checkLengthBounds` also calls `collectExpressionSites`, which walks
fields for the `validation.rule` and `default` expression sites. Only
`checkReservedActionPrefix` and `checkIdResolution` walk no field tree
at all. `checkIdResolution` walks `collectFieldsDeep` directly instead,
covered below.

This change merges only four of the six: `checkPatterns`,
`checkColumnMapping`, `checkFieldKeyFormat`, and the field-key-length
loop inside `checkLengthBounds`. Each of the four runs exactly one
per-field check. Each check reads the same per-field data, with
nothing else in its walk. Folding all four into one pass changes
nothing about what each check reads or reports.

The other two stay separate on purpose, not because they touch no
field tree. The function `checkUnknownKeys` checks every key at every
depth of the whole body, not fields alone. Its field-tree call is one
target among
several sibling calls in the same function, alongside `body.contract`,
`body.dataSources`, and `body.workflow`. Folding it into the
four-check merge would mean threading that whole-body state through a
field-only callback. That callback would carry only one of
`checkUnknownKeys`'s many targets.

`collectExpressionSites` walks fields and workflow steps in one
function, for one shared expression-length bound. Its field-tree half
cannot join the four-check merge without splitting that function's
single field-and-step traversal in two. Both stay out of scope for
that reason. The rest of this Context section covers the four merged
walkers alone.

Three of the four share one traversal, `walkFieldsIndexed`, over
`body.fields`: `checkPatterns`, `checkColumnMapping`,
`checkFieldKeyFormat`. Each one runs a single per-field check.

The fourth of the four, `checkLengthBounds`, runs that same traversal a
...
`checkIdResolution` sits outside the four. It calls `collectFieldsDeep`
...
**D2 (continued): the three checks keep their pre-change names.**
`checkPatterns`, `checkColumnMapping`, and `checkFieldKeyFormat` survive
the merge as separately-named functions. Each still runs exactly the
check it ran before, against exactly one field. Each loses only its own
top-level loop over `body.fields`. `walkFieldsIndexed` calls each of the
three, by name, once per field. It runs them in the fixed sequence D2
already sets: pattern, columnMapping, keyFormat, keyLength.

The field-key-length bound is the exception. It never had a name of its
own. It lived as one loop inside `checkLengthBounds`. It moves into the
same callback as an inline check, with no new named function to match.

This choice is not free-standing. It closes a gap the proposal's
capability-impact search missed. Four artifacts outside this change
cite `checkPatterns`, `checkColumnMapping`, `checkFieldKeyFormat`, or
`checkLengthBounds` by name, in SHALL-worded or otherwise normative
text:

- `openspec/specs/studio-column-mapping-form/spec.md`, lines 53 and
  120, e.g. "`checkColumnMapping` refuses a mapping on a field carrying
  no `dataSource`".
- `openspec/specs/studio-field-validation-form/spec.md`, a full
  Requirement at lines 118-145, naming `compile.ts::checkPatterns` and
  `MAX_PATTERN_LENGTH`.
- `docs/current-state.md`, five citations of `checkPatterns`,
  `checkColumnMapping`, and `contractFieldSchema` by name.
- `.claude/rules/authoring-invariants.md`, which cites `checkPatterns`,
  `checkFieldKeyFormat`, and `checkLengthBounds` by exact name in
  normative rule text. Per `CLAUDE.md`, this file auto-loads for
  `src/schema` and `src/cel` changes. Those are exactly the files this
  change touches.

Every citation names the check a specific per-field function performs.
None describes `compile.ts`'s top-level call sequence. None describes
how many times it walks `body.fields`. Keeping every name intact, as
callable per-field functions performing the identical check, keeps
every citation true after this change ships. Neither
`studio-column-mapping-form` nor `studio-field-validation-form` needs a
delta spec. `docs/current-state.md`'s citations need no change either,
and neither does `.claude/rules/authoring-invariants.md`.

The alternative sets the checks inline, in one anonymous callback body
with no per-check name. This design weighs that alternative and sets it
aside. It would force a delta spec against both studio capabilities. It
would force a `docs/current-state.md` change too, in the same commit,
and a change to `.claude/rules/authoring-invariants.md` besides. Neither
buys any behavior: the merge's purpose is one traversal per field, not
the removal of each check's identity.
...
**[Risk] `simplify-unknown-key-detection`, a sibling OpenSpec change,
deletes `walkViewKeys` outright, which this change's task 4.2 edits.**

`simplify-unknown-key-detection` replaces `checkUnknownKeys` and its 9
`walkFooKeys` functions, `walkViewKeys` among them, with one
schema-driven walker. This change's task 4.2 deletes the `view.renderer`
shape check that lives inside `walkViewKeys` today.

Mitigation: `simplify-unknown-key-detection` applies first (see its own
design.md Migration Plan for the sequencing). Whoever applies this
change's task 1 (re-reading the current baseline) must re-read the
post-`simplify-unknown-key-detection` state of `walkViewKeys` and
`structuralIssues` first. By that point `walkViewKeys` no longer exists
as a named function. The `view.renderer` shape check this change's task
4.2 targets lives inside the new schema-driven walker's object-node
branch instead. Task 4.2 must touch that branch, not a function named
`walkViewKeys`.
...
1. Before implementing the schema change (finding 67), run the audit
   ...
   This repository documents no way to reach a production snapshot or
   read replica (`openspec/specs/deployment-runbook/spec.md` names
   none). No such database may be reachable. Or the person or agent
   applying this change may lack access to one. Either way, do not
   treat that as a zero-row result. Record, in the task checklist, that
   the query could not run and why.

   The Context section's own repo-wide grep already found zero
   producers of `view.renderer` and no example that sets it. That
   finding makes a zero-row outcome the expected one. It is a grep
   result, not the audit query's result. The checklist must record the
   two separately.
