...
`system:publish`, `system:cancel-any`, `system:admin`, and
`system:developer`. Each demo user's email SHALL follow one fixed,
recognizable convention. A re-run SHALL update an existing demo user's
roles and password. It SHALL NOT create a second account with the same
email.

#### Scenario: Provisioning demo users on an empty database
- **WHEN** the seed script runs against a database with no `auth_users`
  rows
- **THEN** four demo users exist afterward, one per reserved role, each
  reachable by its fixed email

#### Scenario: Re-running does not duplicate a demo user
- **WHEN** the seed script runs again after those four users already exist
- **THEN** `auth_users` still contains exactly four demo users, and each
