...
- [x] 1.1 In `src/engine/outbox.ts`, add a `rejectAfter(ms)` helper and race it
  against `deliverFn(row, registry)` at `:179`, using the `leaseMs` parameter
  `drainOutbox` already takes
- [x] 1.2 Let the rejection fall into the existing catch at `:180-183`, so it
  lands in the transient-failure branch with backoff — add no new branch and no
  new state
- [x] 1.3 Clear the deadline timer when the delivery settles, so a completed
  pass does not keep a pending timer alive per delivered row
- [x] 1.4 Comment the abandonment explicitly: `Promise.race` does not cancel
...
- [x] 2.1 In `src/handlers/http.ts`, add a module constant for the default
...
- [x] 4.1 In `src/engine/timers.ts`'s per-instance `catch` (`:53-56`) and the
  ...
  pass read
- [x] 4.2 Do **not** push on the "no due timer" path — that is a normal
  outcome, not a failure
- [x] 4.3 In `src/engine/resolution.ts`, remove the `requeue` calls at `:86`
  and `:100` and leave the row `claimed`, so the existing lease predicate at
  `:63` is the retry cadence
- [x] 4.4 Update the comment at `:72-74`, which documents the removed requeue
  as deliberate
- [x] 4.5 Confirm the writeback's `resolve_state = 'pending'` re-flag still
...
- [x] 6.4 Check the third caller path (`evalOutputMap` at `eval.ts:205-206`,
...
- [x] 7.1 Add `mapping.entry-dropped` to the `InstanceEvent` union in
...
- [x] 7.2 Keep the payload `.strict()`, matching every sibling kind
- [x] 7.3 Confirm the admin record view renders an unknown-to-it kind
  gracefully, or add the kind to its rendering — `packages/admin` reads the
  merged record

## 8. Tests

- [x] 8.1 A handler that never settles: the drain pass completes, the row is
  ...
  none
- [x] 8.2 Unrelated rows, including a `core.spawnSubprocess` row, are
  delivered in the presence of a hung row
- [x] 8.3 A row claimed and abandoned (lease expiry, no tx2) shows an
  incremented `attempts`, and repeated abandonment dead-letters it
- [x] 8.4 `http.request` with no declared `timeout` against a never-responding
  capture server aborts at the default; a server that stalls after headers is
  also aborted
- [x] 8.5 An over-size response dead-letters and writes nothing into `data`
- [x] 8.6 A failing timer instance is not re-selected on the next pass, and a
  concurrently re-armed timer survives the push
- [x] 8.7 A failing resolution instance is not re-claimed until its lease
  expires, and other claimed instances in the same pass still process
- [x] 8.8 A handler returning `"5"` for a `number`-typed output target leaves
  `data` unchanged for that field, marks the row delivered, and records the
  drop in the outcome
- [x] 8.9 A subprocess `inputMapping` over an unset optional field spawns the
  child without that field and records a `mapping.entry-dropped` event; the
  mirrored `outputMapping` case on return
- [x] 8.10 A fully evaluable mapping records no event (the negative case that
  keeps the drop from becoming routine)

## 9. Documentation

- [x] 9.1 `docs/current-state.md`: the outbox entry (deadline, claim-time
...
- [x] 9.2 `CLAUDE.md`: the runtime-record paragraph enumerates the event kinds
  and states which carry `ActionOutcome`s — add `mapping.entry-dropped` to
  both lists
- [x] 9.3 `CLAUDE.md`: the guard-totality rule and the mapping fatality
  currently contradict each other; after this change they agree, so state the
  mapping rule beside it

## 10. Verification

- [x] 10.1 Run `bun run typecheck` from the repo root and confirm it passes
- [x] 10.2 Run the FULL `bun test` suite with `DATABASE_URL` set, from the
  repo root, and confirm it passes — check the skip count, not only the pass
  count
- [x] 10.3 Verify each new test fails without its fix, on a scratch copy of the
  tree — never by mutating the shared working tree
- [x] 10.4 Confirm the existing interleaved-transaction outbox tests
