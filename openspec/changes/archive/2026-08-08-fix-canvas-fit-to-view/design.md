...
by the toolbar's bottom edge plus a gutter. Its bottom edge, not its height.
The toolbar sits `--space-2` below the canvas top. An inset of the height
alone leaves it overlapping by those 8px. The other three edges take the
gutter alone.

`.canvas-toolbar` is a flex row, and it is where a second overlay control
belongs. Measuring the container rather than the button therefore covers
whatever the toolbar grows to hold.
...
The component measures that edge rather than naming a second constant. The
`.btn` rule derives its height from a 14px font and `--space-2` of padding. A
change to either token moves the edge. A hardcoded number does not follow it.
A measurement costs no more code than the constant it replaces.
