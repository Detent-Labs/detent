...
- [x] 1.1 Replace the three summary-line cases in `studio-stepsRail.test.tsx`
  ...
  any source change.
- [x] 1.2 Remove the line from `StepsRail.tsx`, per design.md's decisions on
...
- [x] 1.3 Remove `railRowSummary` from `panels/stepRailRow.ts`, with every
...
- [x] 1.4 Remove the five `stepsRail.` keys design.md lists from
  `catalogs/studio.ts`. Verify: a grep over `packages/web` for each key finds
  no match.

## 2. Prose

- [x] 2.1 Correct the `processLabel` docblock in `draft/guided-labels.ts`.
...
- [x] 2.2 Add a `docs/browser-checks.md` section for this change, shaped like
