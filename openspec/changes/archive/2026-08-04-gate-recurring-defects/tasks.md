...
- [x] 1.3 Handle an all-zero remote sha (new branch): print the merge-base with
  `origin/main`. Use the empty-tree hash when they share no commit. Emit `A..B`
  in every case: `git diff` takes endpoints, not rev-list syntax
...
- [x] 2.7 In `whitespace.sh`, read the CR rule from `git ls-files --eol`, not a grep.
  MSYS grep opens a file in text mode and strips the CR. `grep -lI $'\r'` therefore
...
- [x] 2.10a Rewrite the container users `node` and `root` out of each matched line,
  then re-apply the pattern. A document quoting `/home/node/.claude` then passes.
  A real machine path beside it still fails. Exclude `.gitignore` by path
...
- [x] 6.5 Run the whole hook end to end, and report what each stage printed
