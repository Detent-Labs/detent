...
- [x] 1.1 Add a `groups` table to `initSchema` (`src/engine/store.ts`):
      ...
      decision).
- [x] 1.2 Create `src/auth/groups.ts`, mirroring `src/auth/users.ts`'s
...
- [x] 1.3 `createGroup(name, scope, db)`: mints a `group_` id
      (`crypto.randomUUID()`), inserts with an empty `members` array,
      returns the created row.
- [x] 1.4 `listGroups(page, db)`: keyset pagination on `(name, group_id)`,
      mirroring `listUsers`'s shape and its `DEFAULT_LIST_LIMIT`/
      `MAX_LIST_LIMIT` bounds.
- [x] 1.5 `renameGroup(groupId, name, db)`, `setGroupMembers(groupId,
...
- [x] 1.6 `getGroupMembers(groupId, db)`: the live-resolution read
...
- [x] 1.6b `getGroupScopes(groupIds: string[], db): Promise<Map<string,
...
- [x] 1.7 `referencingPublishedProcesses(groupId, db)`: an `EXISTS` scan of
      ...
      function" decision).
- [x] 1.8 `deleteGroup(groupId, db)`: calls 1.7 first; when it returns a
...
- [x] 1.9 Test: a created group's `name`, `scope` and `members` round-trip
      through `listGroups`/a direct read.
- [x] 1.10 Test: `listGroups` pagination, mirroring `listUsers`'s own
      paging test.
- [x] 1.11 Test: `setGroupMembers` replaces the whole list (an omitted
      member is dropped).
- [x] 1.12 Test: `setGroupScope` accepts both shapes and round-trips each.
- [x] 1.13 Test: `getGroupMembers` excludes a disabled account and a
      member id naming no `auth_users` row, and returns `[]` for an
      unknown `groupId`.
- [x] 1.14 Test: `deleteGroup` refuses and names the referencing process
...
- [x] 2.1 Add `allowedGroups: z.array(z.string()).optional()` to
      ...
      too.
- [x] 2.2 Confirm every body under `examples/` still parses unchanged (none
      should need editing, since the field is optional). Run the example-
      loading test suite and report the result.
- [x] 2.3 Test: a body declaring no `allowedGroups` parses successfully,
      with `allowedGroups` reading as `undefined`.
- [x] 2.4 Test: a body declaring `allowedGroups` with entries parses,
      preserving the entries verbatim.
- [x] 2.5 Test (hash-stability regression): compute `definitionHash` over a
...
- [x] 2.6 Add `"allowedGroups"` to `KNOWN_KEYS` in
...
- [x] 3.1 Add a structural check to `src/schema/compile.ts`, in the style
...
- [x] 3.2 Append it to `structuralIssues`, so it runs before the
...
- [x] 3.3 Test: a step whose `org.group-members` `groupId` names an entry
...
- [x] 3.4 Test: a step whose `org.group-members` `groupId` is absent from
      `allowedGroups` fails the publish, naming the step and the group id.
- [x] 3.5 Test: the check rejects the violation even on a body that already
...
- [x] 4.1 Add `groupMembersConfigSchema = z.object({ groupId: z.string()
...
- [x] 4.4 Test: a step declaring `org.group-members` resolves the group's
      current member list, both members active.
- [x] 4.5 Test: the resolved list excludes a disabled member.
- [x] 4.6 Test: a `groupId` naming no group resolves to `[]`, and the step
      entry commits with nothing thrown.
- [x] 4.7 Test: a membership change made after publishing the process
      reaches the resolved candidates on the next step entry, with no
      republish.
- [x] 4.8 Test: a config carrying an extra key, or missing `groupId`, is
...
- [x] 5.1 Add a check to `src/engine/definitions.ts` that, for every entry
...
- [x] 5.2 Collect every violation (mirroring `RegistryValidationError`'s
...
- [x] 5.2b Register `GroupScopeValidationError` in `src/http/errors.ts`'s
...
- [x] 5.3 Call it from `publishBody` at the same relative position
...
- [x] 5.4 Test: an `allowedGroups` entry naming a `"global"`-scoped group
      publishes for any process.
- [x] 5.5 Test: an `allowedGroups` entry naming a `"processes"`-scoped
...
- [x] 5.6 Test: an `allowedGroups` entry naming no group in the store fails
      the publish, naming that group id.
- [x] 5.7 Test (the placement proof): publish a body once so its hash is
...
- [x] 5.8 `GroupScopeValidationError`'s new `group-scope-validation` type
...
- [x] 6.1 Add six handlers to `src/http/admin-routes.ts`, each gated by
...
- [x] 6.2 `handleAdminCreateGroup` and `handleAdminSetGroupScope` validate
...
- [x] 6.2b Add `GROUP_NAME_MAX_LENGTH = 200` to `src/auth/groups.ts`,
      ...
      `reason`.
- [x] 6.3 `handleAdminDeleteGroup` maps `deleteGroup`'s three outcomes: 404
...
- [x] 6.4 Register the six routes in `src/http/server.ts`: `GET
...
- [x] 6.5 Test: `GET /admin/groups` lists and paginates.
- [x] 6.6 Test: `POST /admin/groups` creates, and refuses an empty `name`.
- [x] 6.6b Test: `POST /admin/groups` refuses a `name` past
      `GROUP_NAME_MAX_LENGTH` (200 characters) with 400, and creates no
      group.
- [x] 6.7 Test: `PATCH /admin/groups/:groupId/name` renames, and 404s for
      an unknown `groupId`.
- [x] 6.7b Test: `PATCH /admin/groups/:groupId/name` refuses a `name` past
      `GROUP_NAME_MAX_LENGTH` (200 characters) with 400, and leaves the
      group's stored name unchanged.
- [x] 6.8 Test: `PATCH /admin/groups/:groupId/members` replaces the whole
      member list.
- [x] 6.9 Test: `PATCH /admin/groups/:groupId/scope` accepts both scope
...
- [x] 6.10 Test: `DELETE /admin/groups/:groupId` refuses and names the
      blocking process for a referenced group, succeeds for an
      unreferenced one, and 404s for an unknown `groupId`.
- [x] 6.11 Test: an actor lacking `system:admin` gets 403 from each of the
      six routes, with no read or write performed.

## 7. Documentation

- [x] 7.1 Extend `docs/authoring-guide.md`: document `allowedGroups` and
...
- [x] 7.1b Extend `docs/current-state.md`. Two clauses there need the same
...
- [x] 7.1c Extend `docs/decisions.md` (~line 135-136, "Two strategies now
      ship: `\"static\"` and `\"org.manager-of-starter\"`"): name three
      strategies instead of two, citing the same source.
- [x] 7.2 Extend `.claude/rules/authoring-invariants.md`: add the
...
- [x] 7.2b Extend `.claude/rules/process-contract.md`'s Extensibility
...
- [x] 7.2c No implementation task for `assignment-strategy-registry`'s
      ...
      build.
- [x] 7.3 Run the antislop check over all five files, and over this change's
...
- [x] 8.1 Run `bun run typecheck`, and report what it printed.
- [x] 8.2 Run `bun run build`, and report what it printed.
...
- [x] 8.4 Check the skip count against `scripts/gates/skip-floor.txt`.
- [x] 8.5 Run `sh scripts/gates/prose.sh < /dev/null`.
- [x] 8.6 Run `sh scripts/gates/whitespace.sh < /dev/null`.
