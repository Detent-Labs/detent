...
- [x] 1.1 Replace the order case at `studio-stepsRail.test.tsx:118` with the
...
- [x] 1.2 Add a case to `studio-stepsRail.test.tsx` asserting each row carries
...
- [x] 1.3 Make `StepsRail.tsx` list `draft.workflow.steps` and drop its
...
- [x] 1.4 Make `EditScreen.tsx` build `railOrder` from the draft's own order.
...
- [x] 1.5 Make `formCardRows.ts` order the cards by the draft's own order.
...
- [x] 1.6 Remove `registerOrder` from `draft/registerOrder.ts`, keeping
...
- [x] 2.1 Correct every passage in `docs/current-state.md` that names the
  rail's old order. Verify: a grep for "reachability" over that file names
  only the canvas bar's report.
- [x] 2.2 Add a section to `docs/browser-checks.md` for this change, shaped
...
- [x] 2.3 Remove the RAIL-1 entry from `docs/decisions.md`, which this change
  closes. Verify: a grep for "RAIL-1" over the repo finds only this change's
  own files.
- [x] 2.4 Correct the `steps rail` row of the chrome table in
...
- [x] 3.1 Run `bun run typecheck`, then `bun run build`, in the devcontainer.
...
- [x] 3.4 Run the `docs/browser-checks.md` entry from 2.2 in a real browser, at
  1440 and 420 pixels wide. Record what each width showed.
- [x] 3.5 Run the Impeccable detector on `StepsRail.tsx`. Then run
