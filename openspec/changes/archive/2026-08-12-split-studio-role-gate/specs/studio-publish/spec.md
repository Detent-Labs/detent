...
`POST /drafts/:processId/publish` SHALL need `system:publish` together with one
of `system:developer` and `system:author`. Neither authoring role implies
anything else, as `process-drafts` already established. Publishing from Studio
therefore stays gated exactly as publishing from anywhere else.
