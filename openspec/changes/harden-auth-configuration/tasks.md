...
- [ ] 1.1 In `src/http/server.ts::resolveAuthResolver`, add the
...
- [ ] 3.1 In `src/auth/login.ts::checkAndRecordAttempt`, before the capacity
  branch at `:54`, delete every entry whose `t - windowStart > WINDOW_MS`
- [ ] 3.2 Change the capacity branch to return `"limited"` when the map is
  still full after the sweep
- [ ] 3.3 Keep the whole function synchronous and `await`-free — the comment
  at `:39-45` states why, and it stays true
- [ ] 3.4 Update the fail-open sentence in that comment and at `:54` to
  describe the new behavior; the current wording documents the removed rule

## 4. Equalize the login timing

- [ ] 4.1 In `src/auth/users.ts`, add a module-scope
  ...
  `src/auth/cli.ts`
- [ ] 4.2 Rewrite `verifyLogin`'s tail so exactly one
...
- [ ] 4.3 Correct the doc comment at `users.ts:20-24`: it currently claims
...
- [ ] 5.1 Add a `transformIndexHtml` plugin with `apply: "build"` to
...
- [ ] 5.2 Repeat for `packages/admin`, `packages/studio`, `packages/editor` —
  four near-identical blocks; do not extract a shared package for five lines
- [ ] 5.3 Policy value: `default-src 'self'; script-src 'self'; style-src
  'self' 'unsafe-inline'; img-src 'self' data:; connect-src <derived>;
  object-src 'none'; base-uri 'none'; form-action 'self'; frame-ancestors
  'none'`
- [ ] 5.4 Run `bun run build` in each package and confirm the meta tag is in
  `dist/index.html` with the expected `connect-src`
- [ ] 5.5 Run `bun run dev` in one package and confirm no policy is injected
  and react-refresh still works

## 6. Devcontainer and documentation

- [ ] 6.1 Add `ALLOW_INSECURE_DEV_AUTH: "1"` to the app service's environment
...
- [ ] 6.2 `docs/current-state.md`: update the auth-configuration entry
  (`:548-556`) — the fallback is now an explicit flag, `AUTH_JWT_SECRET` has
  a minimum length, and the rate limiter fails closed
- [ ] 6.3 `docs/current-state.md`: correct the rate-limit entry (`:614-625`),
  which currently describes the fail-open behavior
- [ ] 6.4 `README.md`: document the three variables and the flag next to the
  run instructions, including `openssl rand -base64 32`

## 7. Tests

- [ ] 7.1 `resolveAuthResolver` throws when nothing is configured and the flag
  is absent, and returns `devHeaderResolver` when the flag is `"1"`
- [ ] 7.2 `resolveAuthResolver` throws for a 31-byte `AUTH_JWT_SECRET` and
  accepts a 32-byte one
- [ ] 7.3 Update the assertion at `test/auth-server.test.ts:33`, which pins
...
- [ ] 7.4 Lengthen `test/auth-server.test.ts:17`'s `SECRET` (currently
...
- [ ] 7.5 `checkAndRecordAttempt`: at capacity with expired windows, a new
...
- [ ] 7.6 `handleLogin` end-to-end: a capacity-full map produces 429 and does
  not reach `verifyLogin`
- [ ] 7.7 `verifyLogin` does the same work for an unknown email as for a
...
- [ ] 7.8 Confirm no suite depended on `createServer`'s removed default

## 8. Verification

- [ ] 8.1 Run `bun run typecheck` from the repo root and confirm it passes
- [ ] 8.2 Run the FULL `bun test` suite with `DATABASE_URL` set, from the
  repo root, and confirm it passes — check the skip count, not only the pass
  count
- [ ] 8.3 Start the server with no auth variables and confirm it exits with
  the naming error; start it with the flag and confirm the warning appears
- [ ] 8.4 Verify each new rejecting test fails without its fix, on a scratch
