...
### Requirement: The users screen edits a manager in place

The admin users screen SHALL show each account's manager. An operator SHALL
change it there, beside the roles control the screen already carries.

The control SHALL offer the other listed accounts as choices. It SHALL offer a
choice clearing the manager. It SHALL NOT offer the account being changed.

A rejected change SHALL leave the displayed value as it was. It SHALL show the
server's message. A successful change SHALL show the saved value without a full
reload.

#### Scenario: An operator changes a manager in place

- **WHEN** an operator picks another account as a manager and confirms
- **THEN** the row shows the new manager, and a reload shows the same value
...
#### Scenario: A rejected change is shown

- **WHEN** a manager change is rejected by the server
- **THEN** the row shows the previous manager and the operator sees the failure
