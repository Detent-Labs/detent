...
- [x] 6.1 Run `bun run typecheck` and then `bun run build` in the devcontainer. Verify: both exit 0.
- [x] 6.2 Run the full `bun test` with `DATABASE_URL` set, in the devcontainer. Pipe its output through `scripts/gates/silent-green.sh`. Verify: no test fails, and the gate exits 0.
- [x] 6.3 Run `sh scripts/gates/range.sh < /dev/null | sh scripts/gates/prose.sh` over the change's range. Run the same range through `scripts/gates/whitespace.sh`. Verify: both gates exit 0, and neither prints "nothing to check" or "SKIPPED".
