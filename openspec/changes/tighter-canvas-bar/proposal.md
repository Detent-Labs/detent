...
while a selection matches a group. The rest of the time the bar carries 36px
of empty space above Add step and 24px below it.
...
- The bar drops its fixed 5.5rem and stands one control row tall, 56px. The
...
- `studio-canvas`: the Canvas tab's layout, the canvas bar's add controls and
  its selection controls change.
...
- `packages/web/src/i18n/catalogs/studio.ts`: one key for the single-step
  label.
- `packages/web/test/studio-canvasBar.test.tsx`: the menu's entries and the
  one-step selection.
