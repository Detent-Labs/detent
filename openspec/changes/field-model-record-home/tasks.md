...
- [ ] 1.1 Append a bullet on the `richtext` format, from D7
      ...
      exits 0.
- [ ] 1.2 Append a bullet on the deferred `image` and `signature` formats, from
      ...
      exits 0.
- [ ] 1.3 Append a bullet on the deferred `slider` and `stars` controls, from
      ...
      exits 0.
- [ ] 1.4 Append a bullet on the `items` key, from D3
      ...
      exits 0.
- [ ] 1.5 Append a bullet for S3, catalog scope, from
      ...
      exits 0.
- [ ] 1.6 Append a bullet for S4, hierarchical option sets, from
      ...
      exits 0.
- [ ] 1.7 Confirm all six bullets sit above the `## Decided and built` heading
...
- [ ] 1.8 Lint the file. Its gate count is 0 today, so the new lines must be
...
- [ ] 2.2 Confirm the file landed beside change 1's own artifacts. Verify:
...
- [ ] 3.0 Record the base count of the four files this section touches, at
      ...
      holds them.
- [ ] 3.1 Point `CLAUDE.md:331`'s bullet at the archived path, under
      ...
      prints 1.
- [ ] 3.2 Replace the span at `docs/current-state.md:17`, inside the
      ...
      prints 1.
- [ ] 3.3 Replace the span in both provenance pointers in `docs/decisions.md`,
      ...
      prints 2.
- [ ] 3.4 Replace the span in all three pointers in `docs/roadmap-history.md`,
      in stages 54, 55 and 56. Verify:
      `grep -c 'field-model-type-format-control/field-model-redesign.md' docs/roadmap-history.md`
      prints 3.
- [ ] 3.5 Re-lint the four worktree files and compare each against its 3.0
...
- [ ] 4.1 Record the base count of all seven files this section touches, at
...
- [ ] 4.2 In `openspec/changes/archive/2026-08-30-field-model-type-format-control/`,
      ...
      prints 0.
- [ ] 4.3 In `openspec/changes/archive/2026-08-31-field-model-person-format/`,
      ...
      prints 0.
- [ ] 4.4 In `openspec/changes/archive/2026-09-01-field-model-view-note/`, write
      ...
      prints 0.
- [ ] 4.5 Re-lint each edited archived file and compare against its 4.1 base.
...
- [ ] 5.1 Confirm no tracked file outside this change's own directory still
      ...
      prints 0.
- [ ] 5.2 Confirm nothing outside Markdown changed. `git mv` stages both halves
      ...
      prints 0.
- [ ] 5.3 Run the prose gate over the pushed range. The rename costs zero,
      ...
      0.
- [ ] 5.4 Run the whitespace gate over the same range. Verify:
      `sh scripts/gates/range.sh < /dev/null | sh scripts/gates/whitespace.sh`
      exits 0.
- [ ] 5.5 Validate the change. Verify:
...
- [ ] 6.4 Confirm no browser check applies: no file under `packages/` changed,
