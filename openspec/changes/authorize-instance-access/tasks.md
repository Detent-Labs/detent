...
- [ ] 1.1 In `src/runtime/api.ts`, import `isEligibleCandidate` from
...
- [ ] 1.2 Split `getInstanceView`'s load into the two-path shape
...
- [ ] 1.3 After the load, reject with `AuthorizationError` unless one of:
...
- [ ] 1.4 Confirm the rejection happens before `resolveFields` runs, so no
  data source is resolved for a caller who may not read the instance

## 2. Add the assignment-less submit floor

- [ ] 2.1 In `submitAndTransition` (`api.ts:586`), add the `else` arm to the
  ...
  carries `ADMIN_ROLE`
- [ ] 2.2 Keep the check where the claim check already is — before
  `validateSubmissionData`, inside the locked transaction — so a rejected
  submission touches nothing
- [ ] 2.3 Leave `claimStep`/`releaseClaim` untouched: both already reject a
  step with no declared assignment

## 3. Tests

- [ ] 3.1 `test/http.test.ts`: `GET /instances/:id` as a third-party
  authenticated actor is 403 with `error.type` `authorization`, mirroring the
  existing record-route 403 test
- [ ] 3.2 `test/http.test.ts`: the same route for a nonexistent instance id,
  as that same actor, is also 403 — the non-disclosure pair
- [ ] 3.3 `test/runtime-api.test.ts`: `getInstanceView` succeeds for the
...
- [ ] 3.4 `test/runtime-api.test.ts`: an actor who was a candidate on a step
  the instance has since left is rejected — advance the instance, then read
  as that actor
- [ ] 3.5 `test/assignment.runtime-api.test.ts`: on a step with no declared
...
- [ ] 3.6 Confirm no existing test relied on reading an instance view as an
...
- [ ] 4.1 `docs/current-state.md`: in the authorization/runtime-API entries,
  ...
  behavior
- [ ] 4.2 Check `README.md` and `packages/app`'s notes for any statement that
  an instance view is readable with a token alone; correct it if present

## 5. Verification

- [ ] 5.1 Run `bun run typecheck` from the repo root (engine plus every
  workspace package) and confirm it passes
- [ ] 5.2 Run the FULL `bun test` suite with `DATABASE_URL` set, from the
...
- [ ] 5.3 Verify each new rejecting test actually fails without the fix, on a
