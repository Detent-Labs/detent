...
- [ ] 1.1 Add a `describe` block to `packages/web/test/studio-guidedSurfaceStyle.test.ts`.
...
- [ ] 1.2 Give the `matrix` style in `FieldMatrixPanel.tsx` a `minHeight: 0`.
  Verify: `bun run typecheck` passes.
- [ ] 1.3 In `FieldMatrixGrid.tsx`, swap the `matrixScroll` style's
...
- [ ] 2.1 Rewrite the 32rem sentence in `docs/current-state.md`, near line 4530.
  It now names the 24rem floor and the fill. Verify: the antislop count on
  that file does not rise.
- [ ] 2.2 Add a `docs/browser-checks.md` entry headed "The field matrix's height
...
- [ ] 3.1 Open `it_offboarding`'s Field matrix tab at 1440x900. Pass: the
  grid's bottom edge equals the tab body's. The tab body's `scrollHeight`
  equals its `clientHeight`. Record the toolbar's height.
- [ ] 3.2 On the same grid, scroll down and sideways. Pass: the step header
  row and the field header column hold their place. The toolbar stays above.
- [ ] 3.3 Resize that window to 1440x1200. Pass: the grid grows by the added
  height and shows more rows.
- [ ] 3.4 Open `access_request`'s Field matrix tab at 1440x1200. First confirm
  ...
  `clientHeight`.
- [ ] 3.5 Open `it_offboarding` again at 1440x600. Pass: the grid measures
...
- [ ] 3.6 Run `/impeccable critique` and `/impeccable audit` against the Field
