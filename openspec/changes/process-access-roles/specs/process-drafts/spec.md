...
published version. The actor SHALL also hold `ADMIN_ROLE`, or appear on that
process's Developer list, per the `process-access-roles` capability.
...
`CREATE_ROLE`, on top of `DEVELOPER_ROLE` or `AUTHOR_ROLE`. The Developer-list
check above SHALL NOT apply here: no Developer list exists yet for a process
that does not exist yet. On success the engine SHALL add the calling actor to
the new process's Developer list. Neither the draft nor the Developer-list
entry SHALL persist without the other.
