...
- [ ] 1.1 In `src/schema/compile.ts`, add a single issue type and error class
...
- [ ] 5.1 In the compile-pass block, resolve every `SubprocessSpec.outputMapping`
  key against the process's own recursive field set
- [ ] 5.2 Resolve every `ProcessContract.inputFields` and
  `ProcessContract.outputFields` entry the same way
- [ ] 5.3 Do **not** add these to the base `processBody` superRefine beside the
...
- [ ] 6.1 Enforce `/^[a-z_][a-z0-9_]*$/` on every `FieldDef.key` in the
  compile-pass block, at full recursive depth
- [ ] 6.2 Declare the length limits as named constants in one place, with a
...
- [ ] 6.3 Enforce them in the same walk, reporting located issues
- [ ] 6.4 Leave `Step.key` and `Path.key` unconstrained — nothing reads them
  as identifiers, and constraining them would force existing bodies to change

## 7. Transport and draft bounds

- [ ] 7.1 Pass `maxRequestBodySize` to `Bun.serve` in `src/http/server.ts:380`,
  as a named constant with a comment naming what it is sized against
- [ ] 7.2 In `src/engine/drafts.ts::checkEnvelope`, add a serialized-size check
  over `body` + `layout`, raising the existing `RequestShapeError`
- [ ] 7.3 Update the `process-drafts` rationale in code comments: the "no
  draft-specific limit because publish has none either" reasoning is no longer
  true once 7.1 lands

## 8. Fixtures, examples and Studio

- [ ] 8.1 Re-publish every body under `examples/` through the new checks and
  correct anything they reject — a fixture that only worked because of
  stripping is documenting the wrong contract
- [ ] 8.2 Do the same for the test fixtures under `test/`
- [ ] 8.3 Check `packages/studio`'s live-validation surface: it runs the
...
- [ ] 9.1 SEC-3 regression in the **additive** shape: a body that *adds* a
  ...
  case beside it
- [ ] 9.2 A body with a misspelled `gaurd` key is rejected, with the located
  path in the issue
- [ ] 9.3 A body with unknown keys in two places reports both
- [ ] 9.4 An uncompilable `validation.pattern` (`"("`) is rejected at publish;
  a valid one still publishes
- [ ] 9.5 A submission whose value violates `maxLength` does not evaluate the
  pattern — assert via a field declaring both, with the pattern's evaluation
  observable through the reported issue set
- [ ] 9.6 A non-identifier `FieldDef.key` (`""`, `"my-field"`, `"2fa"`) is
  rejected; `total_amount` publishes
- [ ] 9.7 An unresolvable `outputMapping` key and an unresolvable
  ...
  `Action.output` suite
- [ ] 9.8 An over-long `Expression.src` is rejected; the repo's examples raise
  no length issue
- [ ] 9.9 An over-size draft envelope raises `RequestShapeError` and leaves
  the stored draft untouched
- [ ] 9.10 Re-publishing a legitimately compiled body is still the no-op it is
  today (the idempotent path must survive every check added here)

## 10. Documentation

- [ ] 10.1 `docs/current-state.md`: record the compile-pass check block and
  each new invariant in the definition-contract/compile entries
- [ ] 10.2 `CLAUDE.md`'s authoring-time invariant list: add the new
...
- [ ] 11.1 Run `bun run typecheck` from the repo root and confirm it passes
- [ ] 11.2 Run the FULL `bun test` suite with `DATABASE_URL` set, from the
  repo root, and confirm it passes — check the skip count, not only the pass
  count
- [ ] 11.3 Verify each new rejecting test fails without its fix, on a scratch
  copy of the tree — never by mutating the shared working tree
- [ ] 11.4 Confirm no already-published fixture body becomes unreadable:
