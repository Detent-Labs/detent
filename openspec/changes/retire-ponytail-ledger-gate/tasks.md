...
- [ ] 1.1 Delete `scripts/gates/ponytail-ledger.sh` and
...
- [ ] 1.2 Delete line 50 of `.githooks/pre-push`, which reads
...
- [ ] 1.3 In `.github/workflows/check.yml`, delete the two-line step at `:50-51`
      ...
      exits 0.
- [ ] 1.4 In `CLAUDE.md`, delete the `ponytail-ledger-fresh` table row at `:195`.
...
- [ ] 2.1 Apply this change's delta to
      ...
      exits 0.
- [ ] 2.2 Confirm the spec file still measures antislop exit 0. Verify:
      `python3 "${ANTISLOP:-$HOME/AI/AntiSlop/antislop.py}" check openspec/specs/push-gate-checks/spec.md`
      exits 0.

## 3. docs/decisions.md

- [ ] 3.1 Add finding 11 as a bullet under the existing heading
...
- [ ] 3.2 Rewrite the SEC-5 bullet per `design.md`'s Decision 8. Drop the
...
- [ ] 3.3 Re-resolve every citation in `design.md`'s Decision 5 against the
...
- [ ] 3.4 Append the new top-level section
...
- [ ] 3.5 Confirm the file still measures antislop exit 0. Its base is 0, so any
...
- [ ] 4.1 In `openspec/specs/field-expression-map-consolidation/spec.md:16`,
      ...
      exits 0.
- [ ] 4.2 In `openspec/specs/engine-poll-loop-consolidation/spec.md:16`, replace
      ...
      exits 0.
- [ ] 4.3 In `openspec/specs/auth-token-lifetime-consolidation/spec.md:15`,
      ...
      exits 0.
- [ ] 4.4 In `openspec/specs/field-tree-check-consolidation/spec.md:11`, replace
      ...
      exits 0.
- [ ] 4.5 In `src/pagination.ts:5`, replace the finding-9 reference with the
      ...
      0.
- [ ] 4.6 In `docs/current-state.md:2657`, replace the finding-9 reference with
      ...
      prints 619 or less.
- [ ] 4.7 Sweep the tree for a surviving reference to either deleted script.
...
- [ ] 5.4 Confirm the hook still runs with one gate fewer. Verify:
...
- [ ] 5.5 Check the worktree bytes of every changed path. Run
...
- [ ] 5.6 Run `openspec validate retire-ponytail-ledger-gate --strict` and
      confirm it exits 0.
