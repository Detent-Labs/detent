...
- [x] 1.3 Handle an all-zero remote sha (new branch): print `<local> --not --remotes`
...
- [x] 2.7 In `whitespace.sh`, read each changed file's worktree bytes for a CR byte,
...
- [ ] 6.1 Run `bun run typecheck` in the devcontainer and report what it printed
- [ ] 6.2 Run the full `bun test` in the devcontainer with `DATABASE_URL` set, and
  report the pass, fail and skip counts
- [ ] 6.3 Run the antislop linter over every Markdown file this change touched
- [ ] 6.4 Run `git diff --check` and the worktree CR probe over the change
- [ ] 6.5 Run the whole hook end to end on the scratch branch, and report each gate's
  line
