...
- [x] 1.1 Add `claimed_by`, `candidates`, `parent_instance_id`,
...
- [x] 1.2 Drop `instances_claimed_by_idx`, `instances_candidates_idx` and
...
- [x] 1.3 Run `initSchema` twice against the devcontainer's Postgres 16 and
  confirm the second run is clean.

## 2. Predicate rewrites

- [x] 2.1 Rewrite `buildInstanceWhere` (`src/runtime/api.ts`): the
...
- [x] 2.2 Rewrite `sweepCancelledChildren` (`src/engine/transition.ts`) to
  filter `parent_instance_id = $1`.
- [x] 2.3 Rewrite the live-child gate in `migrateOne`
...
- [x] 2.4 Rewrite `sweepRetention` (`src/engine/retention.ts`) to compare
  `COALESCE(current_step_entered_at, started_at)` against the `to_char`
  cutoff from `design.md`. Drop both `::timestamptz` casts.
- [x] 2.5 Verify `bun run typecheck` passes after 2.1 through 2.4.

## 3. Tests

- [x] 3.1 Extend `test/instance-column-promotion.test.ts` with a
...
- [x] 3.2 Assert in the same file that `instances_claimed_by_idx`,
  `instances_candidates_idx` and `instances_parent_idx` are absent from
  `pg_indexes` after `initSchema`.
- [x] 3.3 Add one test per new column. Create a real instance, then
  ...
  reads SQL NULL.
- [x] 3.4 Add a test that claims a step, then releases it, and asserts
  `claimed_by` follows `body->'assignment'->>'claimedBy'` both ways with no
  separate write.
- [x] 3.5 Confirm `listInstances` with `assignedTo` still returns what the
...
- [x] 3.6 Add a retention boundary test. Seed one instance whose
...
- [x] 3.7 Add one case to the retention test for a `currentStepEnteredAt`
...
- [x] 3.8 Confirm each new test catches a missing column or a missing
...
- [x] 4.1 Update `docs/decisions.md`'s "Promoting standardized instance
...
- [x] 4.2 Update the index inventory in `docs/decisions.md` around lines
...
- [x] 4.3 Update `docs/current-state.md` around line 2387. It names
...
- [x] 5.2 Run the full `bun test` suite with `DATABASE_URL` set, never a
  ...
  raised.
- [x] 5.3 Re-run the measurement against the implemented `initSchema`
  ...
  empty.
- [x] 5.4 Run `sh scripts/gates/range.sh | sh scripts/gates/prose.sh` with
