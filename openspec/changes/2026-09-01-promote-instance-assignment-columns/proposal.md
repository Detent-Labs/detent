...
scalars into generated columns and deferred five keys. Those five are
`assignment.claimedBy`, `assignment.candidates`, `parent.instanceId`,
`currentStepEnteredAt` and `chainedFrom`. `docs/decisions.md` names them
...
- Add five `GENERATED ALWAYS AS (...) STORED` columns to `instances`:
  ...
  `body`. `parseInstance` is untouched.
- Drop `instances_claimed_by_idx`, `instances_candidates_idx` and
  ...
  `parent_instance_id`). The three new indexes carry new names, so a
  database that already holds the old ones converges in one `initSchema`
  run.
- Rewrite four predicates onto the new columns: the inbox predicate and the
  `claimedBy` filter in `buildInstanceWhere` (`src/runtime/api.ts`), the
  child sweep in `src/engine/transition.ts`, the live-child gate in
  `src/engine/migration.ts`, and the retention sweep in
  `src/engine/retention.ts`. Without the rewrite the new indexes serve
  nothing: the planner does not substitute a column for the expression it
  was generated from, which is the lesson Change 1 recorded.
...
within noise, and the five columns widen the heap by 11.6%. The change
buys one predicate vocabulary and retires three expression indexes.
...
- `docs/decisions.md`: two entries. The index inventory near line 68 names
  the three retired indexes, and the "Promoting standardized instance keys"
...
- Out of scope: `instances_selection_idx`, `instances_current_step_idx` and
  ...
  already exists after Change 1, so retiring it needs no new column. That
  is the optional Change 3.
