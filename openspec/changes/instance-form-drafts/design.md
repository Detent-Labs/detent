...
today is `submitAndTransition` (`src/runtime/api.ts`), which validates the
...
`src/engine/drafts.ts` store *process definitions* for the studio, a different
concept. See proposal.md for motivation.
...
input must therefore stay out of `instance.data`.
...
(`fireTimer`), and cancel-to-sink (`cancelInstance`) all call it, each inside
its own transaction. The one non-authored step-changing write is instance
migration: `migrateInstances` → `migrateOne` (`src/engine/migration.ts`)
relocates `currentStepId` by calling `applyStepEntry` directly, not
`commitTransition`, so the clear hook must also live there (see Decisions 3).
...
- Clear the draft automatically when the instance moves, ends, or is redacted.

**Non-Goals:**

- No autosave; the participant saves explicitly.
- No draft on the Start-a-process screen (that creates, it does not resume).
...
- No Player-screen draft preview: `PlayerScreen.tsx` keeps seeding
  `formValues` from `next.fields` only, unchanged, and never reads
  `next.draft`. See Decision 7 for the accepted gap this leaves.
...
- *Write into `instance.data`* (save equals submit-without-moving). Rejected:
  ...
  reporting, and could reshape the form between saves.
...
only that `data` is a plain JSON object within `MAX_DRAFT_ENVELOPE_BYTES`,
then stores it verbatim. No type, option, constraint, rule, or required check.
`submitAndTransition` stays the sole validator. No `revision` column: the
draft is single-writer at a time — the claimant, or the starter or an
`ADMIN_ROLE` holder on an assignment-less step — so a plain last-write-wins
upsert is enough, unlike the studio's multi-author `drafts`.
...
The draft records the instance's `currentStepId` at save time. A draft is
offered only when its `step_id` equals the instance's current step. This gate
is the backstop: even if a clear hook is missed, a stale draft can never be
offered.

The load-bearing clear rule is deletion inside `commitTransition`'s
transaction, after `applyStepEntry` succeeds. Because every authored step
change and every cancel routes through `commitTransition`, one hook clears the
draft on manual submit, the automatic cascade, a transition timer, and cancel
at once.
...
transition that made it non-running already deleted it, and
...
transaction, the same no-op delete the transition hook uses — on every
migration, not just a relocation. A relocation keeps the draft, and a later
loop back to the recorded step re-offers it (the two step ids match again,
which the `step_id` gate cannot prevent); a fieldMap-only migration leaves the
step id unchanged, so the gate passes and the view would offer a draft whose
field ids the remap has made stale. Deleting on every migration closes both.
...
without `commitTransition` and without a delete, but it is safe because
faulting only parks an instance on an all-automatic step it already reached
through a clearing transition — the hop that entered the cascade deleted the
draft first.

### 4. Authorization parity with submit, via a shared predicate

Extract the submit authorization check from `submitAndTransition`
(claimant-only on an assignment-bearing step; starter or `ADMIN_ROLE`
otherwise; `InstanceNotRunningError` when not running) into one helper shared
by `submitAndTransition` and `saveInstanceDraft`, the way
`loadInstanceForActor` and `isEligibleCandidate` are already shared. This keeps
the two predicates from drifting.

### 5. Fold the draft into `getInstanceView`

Add `draft?: { stepId, data, updatedBy, updatedAt }` to `InstanceView`,
present only when a stored draft's `step_id` matches the current step. One
round trip serves the form and its draft; a separate `GET` endpoint would add
a second request and a second client code path for no gain. The field is read
by every caller `getInstanceView` already authorizes (`ADMIN_ROLE`, the
starter, the current claimant, or an eligible candidate), not just the
claimant who saved it — the form draft is the instance's single shared
scratchpad, like the comment thread, and carries no per-actor split (see
Non-Goals).

### 6. `PUT /instances/:instanceId/draft`

Match the existing draft-store convention (`PUT /drafts/:processId`). The
route resolves the actor, calls `saveInstanceDraft(instanceId, data, actor)`,
and returns `200` with `{ updatedBy, updatedAt }`. `data` is the body field;
the server derives `step_id` from the instance, never from the client.
...
`packages/form-ui` needs no change. The TaskScreen addition is a screen
change under `packages/web`, so its implementation routes through the
frontend design skills first, per the repo convention.
...
previews here is what a participant gets. This change knowingly leaves one
gap in that invariant. An author who opens by id an instance with a saved
form draft sees the step's committed `field.value`s in Player. The
participant's Task screen shows the restored draft instead.
...
- [Instance migration relocates `currentStepId` through `applyStepEntry`, not
  `commitTransition`] → cleared there too (Decisions 3), so no orphan; the
  `step_id` gate remains the backstop for any path a clear hook misses.
...
- [A participant expects a draft to survive a loop back to the same step] →
  accepted and specified: one draft per visit, cleared on transition.

## Migration Plan

`initSchema` gains `CREATE TABLE IF NOT EXISTS instance_drafts ...` beside the
existing `drafts` table. No data migration exists and no existing row changes:
the table starts empty and only new saves populate it. Each DB-backed test
suite calls `initSchema()` in its own `beforeAll`, so the table is created in
tests the same way every other table is. Rollback is a plain
`DROP TABLE instance_drafts`; no instance or definition is affected.
