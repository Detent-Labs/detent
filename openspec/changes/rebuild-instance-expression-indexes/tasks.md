...
- [ ] 1.1 In `src/engine/store.ts::initSchema`, drop `instances_selection_idx`,
...
- [ ] 1.2 Replace the three `CREATE INDEX` statements with plain btrees:
...
- [ ] 1.3 Rewrite the comments above all three, naming their readers and the
...
- [ ] 2.1 Rewrite `buildInstanceWhere` (`src/runtime/api.ts`) so `processId`,
...
- [ ] 2.2 Rewrite `resolveVersionCoverage`'s projection
      (`src/runtime/api.ts`) to `SELECT DISTINCT version`. Verify the report
      column-choice tests still pass.
- [ ] 2.3 Rename `assertVersionHasProcessId` to `assertVersionFilter` and add
...
- [ ] 2.4 Update the two doc comments in `src/runtime/api.ts` that name
...
- [ ] 2.5 Rewrite the migration population scan and `findOrphanKeys`
      ...
      still pass.
- [ ] 2.6 Rewrite `liveVersionCounts` (`src/engine/definitions.ts`) so its
      projection, filter and `GROUP BY` all name columns. Verify the
      cross-process validation tests still pass.
- [ ] 2.7 Rewrite `selectInRange`'s process predicate and the bottlenecks
...
- [ ] 3.1 Update the three schema-init index assertions in
      `test/migration.test.ts` to the new names. Verify the file's index tests
      pass in a full-suite run.
- [ ] 3.2 Add a test that `initSchema` over a database carrying the three old
...
- [ ] 3.3 Add a test in `test/runtime-api.test.ts` pinning the integer
...
- [ ] 4.1 Update `docs/current-state.md` where it names
...
- [ ] 4.2 Update `docs/decisions.md` where it lists the six expression
...
- [ ] 5.1 Run `bun run typecheck` in the devcontainer. Verify it prints no
      error.
- [ ] 5.2 Run `bun run build` in the devcontainer. Verify it completes.
- [ ] 5.3 Run the full `bun test` in the devcontainer with `DATABASE_URL`
...
- [ ] 5.4 Run `sh scripts/gates/range.sh < /dev/null | sh scripts/gates/prose.sh`
      and `sh scripts/gates/whitespace.sh < /dev/null`. Verify both print a
      pass.
- [ ] 5.5 Seed a scratch database with enough rows to move the planner off a
