...
- [ ] 1.1 Add `server: { port: 5173, strictPort: true }` to
  `packages/app/vite.config.ts`
- [ ] 1.2 Add `server: { port: 5174, strictPort: true }` to
  `packages/admin/vite.config.ts`
- [ ] 1.3 Add `server: { port: 5175, strictPort: true }` to
  `packages/studio/vite.config.ts`
- [ ] 1.4 Add `server: { port: 5176, strictPort: true }` to
  `packages/editor/vite.config.ts`

## 2. Widen the devcontainer CORS allowlist

- [ ] 2.1 In `.devcontainer/docker-compose.yml`, set `CORS_ALLOWED_ORIGINS`
  to the four assigned origins, comma-separated with no spaces:
  `http://localhost:5173,http://localhost:5174,http://localhost:5175,http://localhost:5176`
- [ ] 2.2 Replace the comment above it: it must name the frontend dev servers
  as a set and state the package↔port mapping, not "Editor Player dev server"
- [ ] 2.3 Confirm the value still parses as the allowlist form and not the
...
- [ ] 3.1 Restart the devcontainer's `app` service so `startHttpServer`
  re-reads `CORS_ALLOWED_ORIGINS` (it is read once at process start)
- [ ] 3.2 Start all four dev servers and confirm each binds its assigned
  port regardless of start order
- [ ] 3.3 For each of the four origins, confirm the engine answers a
...
- [ ] 3.4 Confirm an origin outside the list (e.g. `http://localhost:9999`)
  still receives no `Access-Control-Allow-Origin` header
- [ ] 3.5 Confirm `strictPort` behaves: occupy one assigned port, start that
  package's dev server, and observe it exit with a port-in-use error instead
  of binding a different port

## 4. Documentation

- [ ] 4.1 Record the package↔port mapping in `docs/current-state.md`'s CORS
