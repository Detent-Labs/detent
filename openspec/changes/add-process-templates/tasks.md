...
- [ ] 1.1 Add the `templates` table to `src/engine/store.ts::initSchema`:
...
- [ ] 1.2 Export `MAX_DRAFT_ENVELOPE_BYTES` from `src/engine/drafts.ts`. The
  template store then shares one bound rather than declaring a second.
- [ ] 1.3 Write `src/engine/templates.ts`, modelled on `drafts.ts`. It exports
...
- [ ] 1.4 Write `test/templates.test.ts`. Cover the round trip and a second
...
- [ ] 1.5 Append `TEMPLATES_ROLE` to `DEMO_USERS` in `scripts/seed.ts`, with
...
- [ ] 2.1 Add `TEMPLATES_ROLE = "system:templates"` to `src/auth/authorize.ts`,
  with a doc comment shaped like `DATALISTS_ROLE`'s, naming the read
  asymmetry and stating that the role implies nothing.
- [ ] 2.2 Add four handlers to `src/http/studio-routes.ts`. The `GET` handlers
...
- [ ] 2.3 Register the four routes in `src/http/server.ts` beside the existing
  `/drafts` entries.
- [ ] 2.4 Extend `test/http-studio.test.ts`. Cover one 403 per route per
  missing role. Cover a read by a `system:developer`-only actor, and a write
  that same actor cannot make.
- [ ] 2.5 In `test/http-studio.test.ts`, assert that an actor holding only
  `system:templates` gets 403 from a draft route, a publish, an `/admin/*`
  route and a `/reporting/*` route.

## 3. Shell and per-screen gate

- [ ] 3.1 Widen the studio entry in `packages/web/src/shell/areas.ts` to
...
- [ ] 3.3 Read `ROUTE_ROLE` in `packages/web/src/areas/studio/root.tsx` for
  the navigation and the direct-hit guard, following
  `packages/web/src/areas/admin/root.tsx`.
- [ ] 3.4 Move an actor stranded on the default `processes` route to the
...
- [ ] 3.5 Change the studio empty state so an actor holding neither studio
  role still learns why the area is empty.
- [ ] 3.6 Test the routing logic without a DOM. An actor holding only
...
- [ ] 4.1 Invoke `/frontend-design:frontend-design` and the Vercel skills.
...
- [ ] 4.2 Add the four template calls to
  `packages/web/src/areas/studio/api/client.ts`.
- [ ] 4.3 Write the templates screen: list rows labelled by the body's
...
- [ ] 4.4 Add creating a template from the current draft, and from a published
  version through `stripCompiledContent`.
- [ ] 4.5 Extract the picker's body-choosing logic beside `seededDraftInput`
...
- [ ] 4.6 Turn `+ New process` into a picker offering an empty process or a
  template. The empty branch keeps today's `{ baseLocale: "en" }` seed. State
  in the picker when no template exists.
- [ ] 4.7 Add `system:templates` to `RESERVED_ROLES` in
  `packages/web/src/areas/admin/screens/UsersScreen.tsx`.
- [ ] 4.8 Test the extracted picker logic and the label fallback.

## 5. Documentation

- [ ] 5.1 Record the new role and the templates table in
  `docs/current-state.md`.
- [ ] 5.2 Mark stage 27d DONE in `ROADMAP.md`. Name the change and the new
  capability. Record that the studio area gained a per-screen gate.
- [ ] 5.3 Check whether `docs/authoring-guide.md` states a rule this change
  alters. Change it in the same commit if it does.
- [ ] 5.4 Refresh the `context:` block in `openspec/config.yaml`. It calls the
...
- [ ] 6.2 Run the full `bun test` with `DATABASE_URL` set. Report the pass
  count and the skip count, not the pass count alone.
- [ ] 6.3 Run the antislop linter over every Markdown file this change
  touched.
- [ ] 6.4 Run `git diff --check`. Then run `git ls-files --eol` and read the
  `w/` column for CRLF.
- [ ] 6.5 Walk the change in a real browser. Create a template from a draft.
  Create one from a published version. Seed a process from each. Delete a
  template. Confirm that an actor holding only `system:templates` sees the
  templates screen alone.
