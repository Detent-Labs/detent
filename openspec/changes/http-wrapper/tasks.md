...
- [ ] 1.1 Create `src/http/errors.ts`: a function mapping a thrown value to
...
- [ ] 2.1 Create `src/http/routes.ts` with a handler for
...
- [ ] 3.1 Create `src/http/server.ts`: `createServer(registry, db = sql)`
...
- [ ] 3.2 Add `startHttpServer(registry, db = sql)`: wraps the `fetch`
...
- [ ] 3.3 Add a `main`-style entry point in `server.ts` (run only when the
      module is executed directly) that builds a `Registry` and calls
      `startHttpServer`.
- [ ] 3.4 Add a `"serve"` script to `package.json` running
      `bun run src/http/server.ts`.

## 4. Tests

- [ ] 4.1 Create `test/http.test.ts` (`test.skipIf(!DATABASE_URL)`,
      following the existing DB-backed suite pattern: `beforeAll`
      `initSchema`, `beforeEach` truncate the relevant tables).
- [ ] 4.2 Cover the happy path for each route: create an instance, get its
      ...
      no real port.
- [ ] 4.3 Cover each typed error mapping (422/409×2/500 for `PinMismatch`)
      and the generic 500 fallback (e.g. an unknown `instanceId`).
- [ ] 4.4 Cover the `AutomaticCascadeLoop` → 200-with-faulted-view case.
- [ ] 4.5 Cover the actor-passing mechanism: body field on both `POST`
...
- [ ] 5.2 `bun test` passes with `DATABASE_URL` set (full suite, not just
      the new file — per this repo's rule that a single-file rerun isn't a
      reliable signal).
- [ ] 5.3 Manually run `scripts/demo-expense-approval.ts`'s flow through
