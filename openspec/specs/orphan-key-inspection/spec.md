...
Migration deliberately retains an instance `data` key whose field the target
catalog no longer declares — dropping it would destroy data unrecoverably (see
`instance-migration`). This capability makes those retained keys visible: a
read-only, on-demand scan that reports, for a given published process version,
which of its pinned instances hold a `data` key absent from that version's field
catalog. It does not prune or otherwise mutate anything; deciding whether and how
to act on an orphan stays an operator decision.
