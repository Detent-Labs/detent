# runtime-field-type-check-consolidation Specification

## Purpose

A structural (mechanism-level) requirement over `src/runtime/api.ts`'s
submission-time field-type check, keeping the `BaseFieldType` -> JS-shape
mapping from being maintained twice. This is an implementation-mechanism
constraint, not user-visible behavior — the actual submission-validation
contract it implements is specified by the `runtime-api` capability
