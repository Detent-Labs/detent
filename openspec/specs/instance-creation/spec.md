...
Defines what `createInstance` derives from the target step when a new
instance is created — starting with `status` — so that a creation reproduces
the same consequences a transition into that step would, rather than
assuming defaults that only hold for the common case.
