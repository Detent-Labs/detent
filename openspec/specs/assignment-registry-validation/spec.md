...
Defines authoring-time validation of a step's `assignment.strategy` against
an injected `assignmentStrategies` registry (`Record<string,
AssignmentStrategyDef>`), sibling to the existing action-handler registry
(see `action-registry-validation`). Resolves each declared strategy's `type`
and, when the resolved strategy declares a `configSchema`, checks `config`
against it — so an unresolvable strategy type or an invalid config is a
publish-time defect, never a runtime one.
