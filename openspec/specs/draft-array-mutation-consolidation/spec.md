# draft-array-mutation-consolidation

## Purpose

A structural (mechanism-level) constraint on the editor's root-level-draft-
array UI: `DataSourcesPanel`, `FieldCatalogPanel`, and `StepsPanel` share one
implementation of add-to-array and update-in-array against the root Immer
draft (`addToDraftArray`/`updateInDraftArray` in
`packages/editor/src/draft/draft-array-crud.ts`), instead of independently
maintained, structurally identical `mutate`-callback copies. External
behavior (what each panel renders, how edits update the Draft) is
unaffected — this is a pure, behavior-preserving extraction. Distinguished
from [[array-crud-by-index-consolidation]] by call convention: that
capability covers prop-array sites that return a new array for an
`onChange` callback; this one covers sites that mutate a field on the root
draft in place inside `mutate((d) => ...)`. This capability exists purely
to keep the "don't re-duplicate this" constraint from silently regressing.
Added for `PONYTAIL-AUDIT.md`'s 2026-07-26 scan, finding 1.
