...
An instance left behind — by `reject-and-pin`, or because it holds actions in flight —
SHALL be recorded as a `migration.skipped` `InstanceEvent` naming the source version,
the target version, and the reason. The reason SHALL distinguish the two causes: one
is a property of the rule and will recur, the other is transient and clears on its
own. It SHALL NOT advance `transitionSeq` and SHALL NOT append a `HistoryEntry`.
