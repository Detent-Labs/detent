# field-input-rendering-consolidation Specification

## Purpose

Structural (mechanism-level) requirements over
`packages/editor/src/player/FieldInput.tsx`, keeping its `select`/
`multiselect` option-list rendering and its free-text-fallback input from
being maintained as duplicate copies. These are implementation-mechanism
constraints, not user-visible behavior — the actual Player field-rendering
contract they implement is specified by the `editor-player` capability
("Field rendering covers every BaseFieldType").
