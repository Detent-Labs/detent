...
A row builder over CEL at the studio's two condition sites. A business analyst
can then author a path guard or a view override without writing CEL. The builder
reads an existing condition back by parsing it. A hand-written guard and a built
...
The studio SHALL offer a row builder as the default editor for a path guard. It
SHALL offer the same builder for the three view overrides `visible`, `required`
and `readonly`. Below the rows the builder SHALL show the CEL it produces, on a
read-only line.
