...
Runtime API Layer. No concrete identity provider (JWT/OIDC/session-based
auth) ships in core — only the `ActorResolver` type plus one non-production
dev resolver for local/dev/example use. Enforcement (assignment/claim) and
resolution are deliberately decoupled: enforcement works against whatever
`Actor` it is given, regardless of how it was produced.
