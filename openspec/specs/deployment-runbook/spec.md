# deployment-runbook Specification
...
environment variable those images read, and the operational rules this
...
such piece of state. For each piece it SHALL say what a second process does to
it, and what a restart does to it.
...
changes the schema. The runbook SHALL say why. The schema step at startup
takes no lock, so two processes that run it together can fail.
