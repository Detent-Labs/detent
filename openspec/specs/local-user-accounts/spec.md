...
creating a user or changing a password — those stay CLI-only. Six
account-administration routes under `/admin/users` are the carve-out. They
list users, toggle `disabled`, and assign roles, a manager and a display
name, reachable over HTTP through the `system:admin`-gated
`admin-user-management` capability.
## Requirements
