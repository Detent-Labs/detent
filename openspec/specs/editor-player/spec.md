...
present; `group` as a nested container housing the fields that carry its
key in `ResolvedViewField.group`. A field of type `reference`, `file`, a
`Plugin` envelope type, or any field carrying `field.dataSource` instead of
`field.options` SHALL render as a free-text input, since no dedicated
widget, data-source resolution, reference picker, or file upload exists in
this preview tool.
...
#### Scenario: A dataSource-bound field renders as free text with a note
- **WHEN** a `select`, `multiselect`, or `reference` field declares
  `field.dataSource` instead of `field.options`
- **THEN** the Player renders it as a free-text input and displays an
  inline note that data-source resolution is not yet supported
