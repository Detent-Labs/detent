...
Defines the Bun workspace boundary between the engine package (repo root)
and the new `packages/editor` package: the workspace layout, and the
engine's `exports` map restricting the editor to the contract surface.
...
(`src/engine/registry-check.ts`). No other module path SHALL be resolvable
by an importing workspace member through the package's own name.
