...
This was a **BREAKING** tightening of the three aggregate routes, landed
2026-08-30 as `reporting-aggregate-read-permission`. An actor holding
`REPORTS_ROLE` alone, with no `read` grant and no `ADMIN_ROLE`, lost the
read access it had. An operator restores it with a `read` grant scoped to
the process, or with `ADMIN_ROLE`.
