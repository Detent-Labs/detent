...
roles, manager and display name, gated by `system:admin` alongside every
other `/admin/*` route (`admin-operations-api`). Creating a user and changing
a password remain CLI-only (`src/auth/cli.ts`) — this capability adds no HTTP
path for either. See the `admin-app` capability for the Users screen that
