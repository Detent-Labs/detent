...
Structural (mechanism-level) requirements over `src/http/errors.ts` and
`src/http/routes.ts` that keep three specific duplications from
re-emerging: table-driven HTTP error mapping, one shared try/catch-and-map
wrapper for route handlers with no extra branching, and one implementation
for resolving an `Actor`'s credential from a `Request`. These are
