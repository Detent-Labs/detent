...
actor's own next call to `cancelInstance` would succeed. The result speaks
...
- **WHEN** an actor holding `system:cancel-any`, or a stored `"cancel"` grant
  over the instance's process, calls `getInstanceView` for a running
  instance. That instance rests on a step whose effective `cancellable`
  resolves to `false`
- **THEN** the returned view carries `canCancel: true`
