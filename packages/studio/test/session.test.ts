...
describe("session persistence", () => {
  ...
  it("uses a stable storage key", () => {
    expect(SESSION_STORAGE_KEY).toBe("studio.session");
  });
  it("changing roles carries system:developer", () => {
    const storage = fakeStorage();
    persistSession({ token: "tok_abc", actorId: "user_1", roles: ["system:developer"] }, storage);
    expect(loadSession(storage)?.roles).toEqual(["system:developer"]);
  });
});
