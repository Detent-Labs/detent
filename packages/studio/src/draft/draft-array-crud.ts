import type { Draft as Immer } from "immer";
...
export function addToDraftArray<T>(mutate: Mutate, ensureArray: (d: Immer<Draft>) => T[], item: T): void {
  mutate((d) => {
...
export function updateInDraftArray<T>(mutate: Mutate, getItem: (d: Immer<Draft>) => T | undefined, patch: Partial<T>): void {
  mutate((d) => {
