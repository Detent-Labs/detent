 ...
 * of this package handles per-screen (`describeCaughtError` + a screen-local
 * error state): React error boundaries never see a throw from inside an
...
export class ErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  override componentDidCatch(error: Error, info: ErrorInfo): void {
    console.error("studio: render-time error caught by ErrorBoundary", error, info.componentStack);
  }

  override render(): ReactNode {
    if (this.state.error) {
      return (
        <main className="studio-boundary-fallback">
          <span className="studio-boundary-stamp">System error</span>
