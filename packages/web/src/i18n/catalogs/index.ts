 ...
 * the admin chunk pulls all three.
 *
 * `admin` and `reporting` are absent on purpose. Neither carries a `t()` today,
 * so neither has a key list to override. Their catalog retrofit adds them here.
 */
...
export const OVERRIDABLE_AREAS = ["shell", "app", "studio"] as const;
