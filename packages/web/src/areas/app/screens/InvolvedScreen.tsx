...
export function InvolvedScreen({ token, locale, navigate, onUnauthorized }: InvolvedScreenProps) {
  ...
  return (
    <main className={`app-tasks ${screenProps.className}`} style={screenProps.style}>
      ...
      {items.length === 0 && !loading && !error && <p {...stylex.props(styles.empty)}>{t(locale, "involved.empty")}</p>}
        ...
        <ul {...stylex.props(styles.taskList)}>
          {items.map((item) => (
            <li key={item.instanceId} {...stylex.props(styles.taskRow)}>
              ...
              {/* The style is named `startedDate` and serves both lists: one row shape, one style. */}
