...
export function describeError(error: Exclude<ClientError, { type: "validation" }>, locale: UiLocale): ErrorOutcome {
  switch (error.type) {
    ...
    default:
      ...
      // variants only another area provokes (draft conflicts, publish
      // rejections). A participant screen cannot reach them; if one ever
      // arrives it reads as a generic failure rather than crashing.
      return { kind: "explain", message: t(locale, "error.generic") };
  }
}
