    ...
    workflow: {
      steps: [
        ...
        { id: "step_archive" as never, key: "archive", type: "task", label: {} },
        {
          ...
          key: "handover",
          type: "task",
          label: { en: "Confirm Completion to Contoso", de: "Abschluss an Contoso bestätigen" },
        },
      ],
    },
  };
...
describe("a label too long for its node", () => {
  // The defect: "Confirm Completion to Contoso" ran out past the node's right
  ...
  it("keeps the whole label in the box the clamp reads", () => {
    ...
    expect(ENGLISH[2].label).toBe("Confirm Completion to Contoso");
  });
  ...
  it("carries the untruncated label as the box's title, before measurement", () => {
    expect(attr(ENGLISH[2].box ?? "", "title")).toBe("Confirm Completion to Contoso");
    expect(attr(ENGLISH[0].box ?? "", "title")).toBe("Capture the request");
    expect(attr(GERMAN[2].box ?? "", "title")).toBe("Abschluss an Contoso bestätigen");
  });
