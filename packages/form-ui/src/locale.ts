 ...
 */
export function resolveFieldsLocale(fields: ResolvedViewEntry[], locale: LocaleCode, baseLocale: LocaleCode): ResolvedViewEntry[] {
  return fields.map((f) => {
    if (!isResolvedViewField(f)) {
      return { ...f, text: { [locale]: resolveText(f.text, locale, baseLocale) } };
    }
    return {
      ...f,
