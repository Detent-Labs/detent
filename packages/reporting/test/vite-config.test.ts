/**
 * Content-Security-Policy meta tag injected at build time only, plus the fixed
 * dev port. Verifies the plugin's shape directly instead of running a full
 * `vite build`, so a regression here fails fast in `bun test`.
 */
import { test, expect, afterEach } from "bun:test";
import config, { contentSecurityPolicy } from "../vite.config.js";
...
test("the dev server pins port 5176 and refuses to fall back", () => {
