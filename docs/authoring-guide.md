...
Every path needs a non-empty `key` and a non-empty `label`, whether manual or
automatic. The studio derives both for you at creation time, from the source
and target step's own names, and you can rename either afterward. A manual
path's `label` is not just an inspector name — it is the text on the button a
participant clicks to take it.
