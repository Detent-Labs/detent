...
- HTTP wrapper (`src/http/`, `test/http.test.ts`, roadmap #5b): a thin REST/JSON
  ...
  **CORS is configuration, not a constant** (`configurable-cors-origins`):
  ...
  The devcontainer's `app` service derives
  `CORS_ALLOWED_ORIGINS` from `PORT_VITE`
  (`pin-frontend-dev-ports`, narrowed by `consolidate-frontend-shell`, then
  made per-checkout by `per-worktree-devcontainer-stacks`). It carries both
  ...
  Its `vite.config.ts` pins the in-container port to 5173, with
  ...
  checkout's own `PORT_VITE`. A linked worktree's frontend dev server then
...
- Notifications (`src/handlers/notification-email.ts`, roadmap #16): a second
  ...
  compose file publishes no host port for it, matching `db`. Each
  checkout's bring-up (`bash scripts/dev-up.sh`) publishes its own derived
  `PORT_MAILPIT` into the gitignored, machine-generated
  `docker-compose.ports.yml` (`per-worktree-devcontainer-stacks`).

  A contributor's own `docker-compose.override.yml` stays free for an extra
  binding of their own. The end-to-end test reads messages back over
  Mailpit's HTTP API inside the compose network. It never depends on a host
  binding.
...
- Authorization (`src/auth/authorize.ts`, `add-authorization`): closes the gap
  ...
  assignment candidate for. The later change `test-instance-claim-bypass`
  gave `claimStep` one exception. A `system:admin` holder now also claims a
  test instance's step outside its candidate list. Granting the roles needs no new tooling —
...
the fifth participant route. It sends no actor id: the engine resolves the
...
The access half already worked. `loadInstanceForActor` admits the starter,
unless a revocation names them (`instance-visibility-view`). A
...
`instances`. The second lets the owner role create the two triggers
below. The engine's own connecting role owns `instances`, not
`detent_audit_owner`.
...
actor. The file `docs/decisions.md` records the resulting gap: a candidate
can claim a test instance it cannot open.
