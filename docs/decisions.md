...
- **Instance data tables: a report builder over instance field values.** A
  ...
  **Open, deliberately.** A download of the table (CSV for a department,
  ...
  structure never changes out of `body` into real columns. It is worth its
  own change and helps more than this feature. `data` itself cannot
  follow: its key set belongs to a process version. The full column
  inventory is under Open questions, above.
