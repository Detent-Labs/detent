...
exists. This file holds open questions, decisions not yet built, the open
findings of past code reviews and design audits, and refused
simplifications. The reasoning behind a decision that has since shipped
lives in `docs/decisions-archive.md`. A change that builds a decided entry
below moves that entry into the archive, word for word. `ROADMAP.md` carries
