...
const handler = () => resolveDataSource(createDefaultDataSourceRegistry(sql), DB_LIST_DATA_SOURCE_TYPE)!;
...
test.skipIf(!DB)("an existing list with no values resolves to an empty option list", async () => {
  ...
  expect(await handler().resolve({ config: { listKey: "empty" } })).toEqual([]);
});
...
test.skipIf(!DB)("an active value resolves to an option, an inactive value nobody holds stays out", async () => {
  ...
  expect(await handler().resolve({ config: { listKey: "cost_centres" } })).toEqual([{ value: "cc1", label: { en: "One" } }]);
});
...
test.skipIf(!DB)("an inactive value heldValues names comes back with its label", async () => {
  ...
  const options = await handler().resolve({ config: { listKey: "cost_centres" }, heldValues: ["cc2"] });
...
test.skipIf(!DB)("options come back ordered by sort_order, then value", async () => {
  ...
  const options = await handler().resolve({ config: { listKey: "ordered" } });
...
test.skipIf(!DB)("a list over the bound raises rather than truncating, naming the listKey", async () => {
  ...
  let raised: unknown;
  try {
    await handler().resolve({ config: { listKey: "big" } });
  } catch (e) {
    raised = e;
  }
  expect(raised).toBeInstanceOf(Error);
...
test.skipIf(!DB)("a list sitting exactly on the bound still resolves for a holder of a retired value", async () => {
  ...
  expect(await handler().resolve({ config: { listKey: "full" } })).toHaveLength(MAX_DATA_LIST_VALUES);
  const forHolder = await handler().resolve({ config: { listKey: "full" }, heldValues: ["retired_held"] });
...
test.skipIf(!DB)("a list one active value over the bound still raises, even for a holder", async () => {
  ...
  let raised: unknown;
  try {
    await handler().resolve({ config: { listKey: "over" }, heldValues: ["retired_held"] });
  } catch (e) {
    raised = e;
  }
  expect(raised).toBeInstanceOf(Error);
...
test.skipIf(!DB)("an unknown listKey raises a plain canary Error naming the key", async () => {
  let raised: unknown;
  try {
    await handler().resolve({ config: { listKey: "no_such_list" } });
  } catch (e) {
    raised = e;
  }
  expect(raised).toBeInstanceOf(Error);
...
test.skipIf(!DB)("the static handler ignores heldValues", async () => {
  ...
  const staticDef = resolveDataSource(createDefaultDataSourceRegistry(sql), "static")!;
  expect(await staticDef.resolve({ config: { options }, heldValues: ["ca"] })).toEqual(options);
});
